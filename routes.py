...
def custom_query():
    db = get_db()
    cur = db.execute('SELECT * FROM salaries ORDER BY TotalPay DESC LIMIT 20;')
    entries = cur.fetchall()
    return render_template('layout.html', content='modules/data.html', status='good', entries=entries)
"""
